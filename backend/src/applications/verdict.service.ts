...
import {
  ...
  GapImpact,
} from './entities/application-verdict.entity';
...
export class VerdictService {
  ...
  async getVerdict(applicationId: string): Promise<ApplicationVerdict> {
    const existing = await this.verdictRepo.findOne({
      where: { applicationId },
    });
  ...
  async submitFeedback(
    ...
    const verdict = await this.verdictRepo.findOne({
      where: { applicationId },
    });
    if (!verdict)
      throw new NotFoundException(
        'No verdict exists for this application yet.',
      );
  ...
  private async computeAndSave(
    ...
    const impactOrder: Record<GapImpact, number> = {
      CRITICAL: 0,
      HIGH: 1,
      MEDIUM: 2,
      LOW: 3,
    ...
    if (ratedCount === 0 && totalComps > 0) {
      riskFlags.push({
        ...
        severity: 'HIGH',
        message:
          'No competencies have been evaluated. Complete the evaluation tab for a reliable verdict.',
      });
    } else if (ratedCount < totalComps) {
      riskFlags.push({
        code: 'INCOMPLETE_EVAL',
        severity: 'MEDIUM',
        message: `${totalComps - ratedCount} of ${totalComps} competencies are still unrated.`,
      });
    }
    ...
    if (gaps.some((g) => g.impact === 'CRITICAL')) {
      riskFlags.push({
        ...
        severity: 'HIGH',
        message:
          'One or more critical competency gaps detected. These are core requirements for this role.',
      });
    }
    ...
    if (interviewRows.length === 0) {
      riskFlags.push({
        ...
        severity: 'LOW',
        message:
          'No completed interviews on record. Schedule an interview to strengthen the verdict.',
      });
    }

    if (!app.skills || app.skills.length === 0) {
      riskFlags.push({
        code: 'CV_PARSE_INCOMPLETE',
        severity: 'MEDIUM',
        message:
