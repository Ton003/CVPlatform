...
export class ScoreSnapshotSubscriber implements EntitySubscriberInterface<ApplicationCompetencyScore> {
  ...
  listenTo() {
    return ApplicationCompetencyScore;
  }
  async afterInsert(event: InsertEvent<ApplicationCompetencyScore>) {
    const app = await event.manager.findOne(Application, {
      where: { id: event.entity.applicationId },
    });
    if (app)
      setImmediate(() =>
        this.snapshotService
          .rebuildSnapshot(app.candidateId, app.id)
          .catch(console.error),
      );
  }
  async afterUpdate(event: UpdateEvent<ApplicationCompetencyScore>) {
    ...
    const applicationId =
      event.entity?.applicationId ?? event.databaseEntity?.applicationId;
...
export class CareerEntrySnapshotSubscriber implements EntitySubscriberInterface<CandidateCareerEntry> {
  ...
  listenTo() {
    return CandidateCareerEntry;
  }
  async afterInsert(event: InsertEvent<CandidateCareerEntry>) {
    setImmediate(() =>
      this.snapshotService
        .rebuildSnapshot(event.entity.candidateId)
        .catch(console.error),
    );
  }
  async afterUpdate(event: UpdateEvent<CandidateCareerEntry>) {
    const candidateId =
      event.entity?.candidateId ?? event.databaseEntity?.candidateId;
    if (candidateId)
      setImmediate(() =>
        this.snapshotService.rebuildSnapshot(candidateId).catch(console.error),
      );
  }
}
...
export class ApplicationSnapshotSubscriber implements EntitySubscriberInterface<Application> {
  ...
  async afterInsert(event: InsertEvent<Application>) {
    setImmediate(() =>
      this.snapshotService
        .rebuildSnapshot(event.entity.candidateId, event.entity.id)
        .catch(console.error),
    );
  }
  async afterUpdate(event: UpdateEvent<Application>) {
    ...
    if (candidateId && id) {
      setImmediate(() =>
        this.snapshotService
          .rebuildSnapshot(candidateId, id as string)
          .catch(console.error),
      );
    }
  }
