...
export class CandidateScoringService {
  ...
  async score(candidateId: string): Promise<CandidateScoreResult> {
    ...
    const skills: string[] = (row?.skills ?? []).map((s: string) =>
      s.toLowerCase(),
    );
    ...
    if (compEntries.length > 0) {
      const totalLevels = compEntries.reduce(
        (sum, c) => sum + (c.level || 0),
        0,
      );
    ...
    const weights = { sfia: 0.6, role: 0.4 };
    const compositeScore = Math.round(
      sfiaScore * weights.sfia + technicalScore * weights.role,
    );
