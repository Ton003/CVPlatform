...
const KNOWN_SKILLS = Object.keys(SKILL_ALIASES).sort(
...
export class KeywordExtractorService {
  ...
  private extractLimit(text: string): number {
    const match = text.match(
      /\b(?:top|give\s+me|show\s+me|find|best|show)?\s*(\d+)\s*(?:candidates?|devs?|developers?|engineers?|results?|matches?)?\b/i,
    );
    if (match) {
