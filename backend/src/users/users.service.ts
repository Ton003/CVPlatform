  ...
  async create(data: any): Promise<User> {
    return this.dataSource.transaction(async (manager) => {
      ...
      if (savedUser.role === 'manager' || savedUser.role === 'admin') {
        ...
        if (!levelId) {
          const levels = await manager.query('SELECT id, "jobRoleId" FROM job_role_levels LIMIT 1');
          levelId = levels[0]?.id;
          roleId = levels[0]?.jobRoleId;
        } else {
          const roles = await manager.query('SELECT "jobRoleId" FROM job_role_levels WHERE id = $1', [levelId]);
        ...
        if (levelId && roleId) {
          ...
          const empRows = await manager.query(`
            ...
            RETURNING id
          `, [empId, savedUser.firstName, savedUser.lastName, savedUser.email, roleId, levelId, departmentId || null, savedUser.id]);
