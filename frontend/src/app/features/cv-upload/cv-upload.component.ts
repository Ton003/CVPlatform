...
export class CvUploadComponent {
  ...
  get canUpload(): boolean {
    ...
    if (this.mode === 'groq' && !this.groqApiKey.trim()) return false;
    return true;
  }
  ...
  setFile(file: File) {
    ...
    // ✅ Client-side size check — backend enforces 10MB too
  ...
  upload() {
    ...
    if (this.mode === 'groq') {
      formData.append('apiKey', this.groqApiKey);
    }

    // ✅ No manual token handling — jwt interceptor adds Bearer header automatically
    ...
    this.http.post<any>(`${environment.apiUrl}/cv-upload`, formData).pipe(
      ...
      error: (err) => {
        if (err.status === 401) {
          ...
          this.error = err.error?.message ?? 'Invalid file or missing fields.';
        } else if (err.status === 0) {
          this.error = 'Cannot reach the server. Make sure the backend is running.';
        } else {
